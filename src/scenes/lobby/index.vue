...
// 设置场景
const setupScene = (threeScene: THREE.Scene, threeCamera: THREE.PerspectiveCamera, threeRenderer: THREE.WebGLRenderer) => {
  scene = threeScene
  camera = threeCamera
...
const createDoors = () => {
  ...
  const doorConfigs = [
    { id: 1, x: -6, color: 0x6B4423 },
    { id: 2, x: -2, color: 0x7B5B3A },
    { id: 3, x: 2, color: 0x8B6B4A },
    { id: 4, x: 6, color: 0x9B7B5A },
  ]
...
const createSingleDoor = (id: number, x: number, color: number) => {
  ...
  // 门框
  const frameGeometry = new THREE.BoxGeometry(3.5, 6, 0.3)
